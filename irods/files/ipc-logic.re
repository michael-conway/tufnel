...
sendMsg(*Topic, *Msg) = ipc_amqpSend(ipc_AMQP_HOST,
                                     ipc_AMQP_PORT,
                                     ipc_AMQP_USER,
                                     ipc_AMQP_PASSWORD,
                                     ipc_AMQP_EPHEMERAL,
...
mkEntityField(*Path) = ipc_jsonString('entity', *Path)
...
sendCollectionInheritModified(*recursive, *inherit, *path) =
  let *msg = ipc_jsonDocument(list(mkAuthorField(),
                                   mkEntityField(*path),
...
sendCollectionAclModified(*recursive, *accessLevel, *username, *zone, *path) =
  let *msg = ipc_jsonDocument(list(mkAuthorField(),
                                   mkEntityField(*path),
                                   ipc_jsonBoolean('recursive', *recursive),
...
sendCollectionAccessModified(*recursive, *accessLevel, *username, *zone, *path) {
  if (*accessLevel == 'inherit') {
    sendCollectionInheritModified(*recursive, true, *path);
  }
  else if (*accessLevel == 'noinherit') {
    sendCollectionInheritModified(*recursive, false, *path);
  }
  else {
    sendCollectionAclModified(*recursive, *accessLevel, *username, *zone, *path);
  }
...
sendDataObjectAclModified(*accessLevel, *username, *zone, *path) =
  let *msg = ipc_jsonDocument(list(mkAuthorField(),
                                   mkEntityField(*path),
                                   ipc_jsonString('permission', *accessLevel),
                                   mkUserObject('user', *username, *zone)))
...
sendDataObjectMetadataModified =
  let *msg = ipc_jsonDocument(list(mkAuthorField(),
                                   mkEntityField($objPath)))
...
sendEntityMove(*EntityType, *Old, *New) =
  let *msg = ipc_jsonDocument(list(mkAuthorField(),
                                   mkEntityField(*Old),
                                   ipc_jsonString('new-path', *New)))
  in sendMsg(*EntityType ++ '.mv', *msg)

sendEntityRemove(*EntityType, *Path) =
  let *msg = ipc_jsonDocument(list(mkAuthorField(), mkEntityField(*Path)))
  in sendMsg(*EntityType ++ '.rm', *msg)

sendAvuMod(*ItemType, *ItemName, *oldName, *oldValue, *oldUnit, *newName, *newValue, *newUnit) =
  let *msg = ipc_jsonDocument(list(mkAuthorField(),
                                   mkEntityField(*ItemName),
...
sendAvuSet(*Option, *ItemType, *ItemName, *AName, *AValue, *AUnit) =
  let *msg = ipc_jsonDocument(list(mkAuthorField(),
                                   mkEntityField(*ItemName),
...
sendAvuMultiremove(*ItemType, *ItemName, *AName, *AValue, *AUnit) =
  let *msg = ipc_jsonDocument(list(mkAuthorField(),
                                   mkEntityField(*ItemName),
...
#
createOrOverwrite { applyAllRules(createOrOverwriteRules, 0, 0); }
createOrOverwriteRules {
  or { if ($writeFlag == 0) { ensureAdminOwner($objPath); }}
	or { msiSysChksumDataObj; }
  or { if ($writeFlag == 0) { assignUUID('-d', $objPath); }}
  or {
	  if ($writeFlag == 0) {
		  sendDataObjectAdd();
	  } else {
...
#
ipc_acPostProcForCollCreate { applyAllRules(ipc_acPostProcForCollCreateRules, 0, 0); }
ipc_acPostProcForCollCreateRules {
  or { ensureAdminOwner($collName); }
  or { assignUUID('-c', $collName); } 
  or { sendCollectionAdd($collName); }
}

# This rule sends a remove collection message for the collection being removed.
ipc_acPostProcForRmColl {
  sendEntityRemove(COLLECTION_TYPE, $collName);
}

# This rule sends a remove data object message for the data object being deleted.
#
ipc_acPostProcForDelete {
  sendEntityRemove(DATA_OBJECT_TYPE, $objPath);
}
...
#
ipc_acPostProcForTarFileReg { applyAllRules(ipc_acPostProcForTarFileRegRules, 0, 0); }
ipc_acPostProcForTarFileRegRules {
  or { ensureAdminOwner($objPath); }
  or { assignUUID('-d', $objPath); }
  or { sendDataObjectAdd(); }
}
...
ipc_acPostProcForPut {
  createOrOverwrite;
#  if ($objPath like "/iplant/home/*" && $objPath not like "/iplant/home/shared/aegis/*" && $objPath not like "/iplant/home/de-irods/de-temp-dir/*") {
#    delay("<PLUSET>1m</PLUSET>") {
#      msiSysReplDataObj("taccCorralRes", "null");
#    }
#  }
}
...
ipc_acPostProcForModifyAVUMetadata(*Option, *SourceItemType, *TargetItemType, *SourceItemName,
                                   *TargetItemName) {
  sendAvuCopy(*SourceItemType, *TargetItemType, *SourceItemName, *TargetItemName)
}
